...
async function main() {
  ...
  const abi = JSON.parse(readFileSync('build/CryptoAgentTasks.abi.json', 'utf8'));
  const bytecode = JSON.parse(readFileSync('build/CryptoAgentTasks.bytecode.json', 'utf8'));

  const factory = new ethers.ContractFactory(abi, bytecode, wallet);
  
  console.log('Deploying CryptoAgentTasks contract...');
